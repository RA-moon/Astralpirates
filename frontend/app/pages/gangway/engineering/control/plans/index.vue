<template>
  <PageShell page-path="gangway/engineering/control/plans" :page-data="null">
    <section class="plan-index">
      <UiStack as="header" class="plan-index__intro" :gap="'var(--space-sm)'">
        <UiHeading :level="1" size="h2">Plan archive & detail pages</UiHeading>
        <UiText>
          Browse the roadmap plans, including archived <strong>tested</strong> and <strong>canceled</strong>
          work. Content is served from Payload with a bundled fallback so editors can update plans
          without shipping new markdown.
        </UiText>
        <UiInline :gap="'var(--space-xs)'" class="plan-index__badges" :align="'center'">
          <UiBadge :value="activePlans.length" variant="info">Active/Queued</UiBadge>
          <UiBadge :value="archivePlans.length" variant="muted">Archive</UiBadge>
      ...
      <UiSurface v-else-if="!isAuthenticated" variant="panel" class="plan-index__guard">
        <UiStack :gap="'var(--space-sm)'">
          ...
          <UiText variant="muted">
            Log in to view the plan archive and detail pages. Plan links stay hidden until you embark.
          </UiText>
          <UiButton @click="openAuthDialog">
            Embark to view plans
          </UiButton>
        </UiStack>
      </UiSurface>
      ...
      <template v-else>
        ...
        <div class="plan-index__lists">
          <UiSurface variant="panel" class="plan-index__list">
            <UiStack :gap="'var(--space-sm)'">
              <UiInline :gap="'var(--space-xs)'" :align="'center'" class="plan-index__list-heading">
                <UiHeading :level="2" size="h4">Active plans</UiHeading>
                <UiBadge :value="activePlans.length" variant="info" />
              </UiInline>
              <UiText variant="muted">
                Includes queued, in-flight, and shipped plans that still need visibility on the roadmap.
              </UiText>
              <ul v-if="activePlans.length" class="plan-index__items">
                <li v-for="plan in activePlans" :key="plan.id">
              ...
              <UiEmptyState
                v-else
                title="No active plans match your filters"
                description="Adjust filters or search to find plans."
              />
            </UiStack>
          </UiSurface>
          ...
          <UiSurface variant="panel" class="plan-index__list plan-index__list--archive">
            <UiStack :gap="'var(--space-sm)'">
              <UiInline :gap="'var(--space-xs)'" :align="'center'" class="plan-index__list-heading">
                <UiHeading :level="2" size="h4">Archive</UiHeading>
                <UiBadge :value="archivePlans.length" variant="muted" />
              </UiInline>
              <UiText variant="muted">
                Plans marked <strong>tested</strong> or <strong>canceled</strong> move out of the roadmap and stay available here.
              </UiText>
              <ul v-if="archivePlans.length" class="plan-index__items">
                <li v-for="plan in archivePlans" :key="plan.id">
              ...
              <UiEmptyState
                ...
                description="Mark shipped items as tested after QA, or cancel plans to move them here."
              />
            </UiStack>
          </UiSurface>
        </div>
...
const isArchivedPlan = (plan: Plan) => plan.status === 'tested' || plan.status === 'canceled';
const activePlans = computed(() => filteredPlans.value.filter((plan) => !isArchivedPlan(plan)));
const archivePlans = computed(() => filteredPlans.value.filter(isArchivedPlan));
...
@media (min-width: calc(var(--size-base-layout-px) * 900 * var(--size-scale-factor))) {
  .plan-index__lists {
