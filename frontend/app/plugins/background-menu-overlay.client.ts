...
export default defineNuxtPlugin(() => {
  ...
  const loadMenuOverlay = async () => {
    ...
    const mountNow = () => {
      const target = document.getElementById('bg-menu-wrap');
      ...
      if (typeof module.mountBackground === 'function') {
        ...
        handle = module.initBackground('#bg-menu-wrap', mountOptions);
      } else {
        return false;
      }
  ...
  watch(
    ...
    (value) => {
      if (value) {
        ...
        handle = null;
        disconnectMountObserver();
        const wrap = document.getElementById('bg-menu-wrap');
        if (wrap) {
          wrap.innerHTML = '';
