...
const REVISIONS_TABLE = sql.raw('"public"."editor_document_revisions"');
...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS ${LOCKS_TABLE} (
      ...
      "holder_user_id" integer NOT NULL REFERENCES ${USERS_TABLE}("id") ON DELETE CASCADE,
  ...
  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS "editor_document_locks_expires_idx"
      ON ${LOCKS_TABLE} USING btree ("expires_at");
  `);
  ...
  await db.execute(sql`
    CREATE UNIQUE INDEX IF NOT EXISTS "editor_write_idempotency_unique_key"
      ON ${IDEMPOTENCY_TABLE} USING btree ("document_type", "document_id", "idempotency_key");
  `);

  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS "editor_write_idempotency_created_idx"
      ON ${IDEMPOTENCY_TABLE} USING btree ("created_at");
  `);
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`DROP TABLE IF EXISTS ${IDEMPOTENCY_TABLE} CASCADE;`);
  await db.execute(sql`DROP TABLE IF EXISTS ${LOCKS_TABLE} CASCADE;`);
  await db.execute(sql`DROP TABLE IF EXISTS ${REVISIONS_TABLE} CASCADE;`);
}
